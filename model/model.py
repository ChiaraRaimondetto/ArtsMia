...
class Model:
    ...
    def _ricorsione(self,parziale,lun):
        if len(parziale)==lun:
            #condizione di termiizazione,allora parziale è lunga esattamente lun
            #per cui verifico che questa parziale sia meglio del mio best
            #(codizione di ottimalità), ed in ogni caso esco
    ...
    def getInfoCompConnessa(self,id_oggetto):
        ...
        print("size connessa con dfs_tree", len(dfsTree))

        #strategia 2
        dfsPred= nx.dfs_predecessors(self._graph,source)
        print("size connessa con dfs_predecessors", len(dfsPred.values())) #la chiave èil node e il valore è la seq di predecessori per arrivare al nodo
